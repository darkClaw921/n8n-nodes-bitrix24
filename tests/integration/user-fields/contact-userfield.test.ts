 ...
 */
const FIELD_SUFFIX = TEST_PREFIX.replace(/[^A-Z0-9_]/gi, '').toUpperCase() + 'CONTACT';
const FIELD_NAME = 'UF_CRM_' + FIELD_SUFFIX;

describe('Contact User Field CRUD', () => {
	let userFieldId: number;

	afterAll(async () => {
		if (userFieldId) {
			await safeDelete('crm.contact.userfield.delete.json', userFieldId, 'id');
		}
	});

	it('should create a contact userfield', async () => {
		const result = await callBitrix24Result('crm.contact.userfield.add.json', {
			fields: {
				FIELD_NAME,
				USER_TYPE_ID: 'string',
				EDIT_FORM_LABEL: {
					ru: testName('ContactUF'),
					en: testName('ContactUF'),
				},
				XML_ID: testName('contactuf'),
			},
		});
	...
	it('should get the created userfield by ID', async () => {
		...
		const result = await callBitrix24Result('crm.contact.userfield.get.json', {
			id: userFieldId,
		});
	...
	it('should list userfields and find the created one', async () => {
		...
		const result = await callBitrix24Result('crm.contact.userfield.list.json', {
	...
	it('should update the userfield EDIT_FORM_LABEL', async () => {
		...
		const result = await callBitrix24Result(
			'crm.contact.userfield.update.json',
			{
				id: userFieldId,
				fields: {
					EDIT_FORM_LABEL: {
						ru: testName('ContactUFUpdated'),
						en: testName('ContactUFUpdated'),
					},
				},
			},
		);
	...
	it('should verify the userfield was updated', async () => {
		...
		const hasUpdated =
			(typeof labels === 'string' && labels.includes('ContactUFUpdated')) ||
			(typeof labels === 'object' &&
				Object.values(labels).some(
					(v: any) =>
						typeof v === 'string' && v.includes('ContactUFUpdated'),
				));
	...
	it('should delete the userfield', async () => {
		...
		const result = await callBitrix24Result(
			'crm.contact.userfield.delete.json',
			{
				id: userFieldId,
			},
		);
	...
	it('should verify the userfield was deleted', async () => {
		...
		try {
			await callBitrix24Result('crm.contact.userfield.get.json', {
				id: userFieldId,
			});
